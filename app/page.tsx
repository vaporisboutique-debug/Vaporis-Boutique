    ...
    <>
      ...
      <section className="container-pad reveal py-16">
        <div className="mb-8 flex flex-col justify-between gap-4 sm:flex-row sm:items-end">
          <div>
            ...
            {isLoading ? (
              <div className="mt-3 h-10 w-72 animate-pulse bg-porcelain" />
            ) : (
              <h2 className="mt-3 font-serif text-4xl">{settings.newestProductsTitle || "Newest Products"}</h2>
            )}
            {isLoading ? <div className="mt-3 h-5 w-full max-w-xl animate-pulse bg-porcelain" /> : settings.newestProductsSubtitle ? <p className="mt-2 max-w-2xl text-sm leading-6 text-ink/62">{settings.newestProductsSubtitle}</p> : null}
          </div>
      ...
      {showBrandStory ? (
        <section className="container-pad reveal grid gap-10 py-16 lg:grid-cols-[0.9fr_1.1fr]">
          {isLoading ? (
            <div className="min-h-[440px] animate-pulse border border-ink/10 bg-porcelain" />
          ) : (
            <div
          ...
          <div className="flex flex-col justify-center">
            {isLoading ? (
              <div className="grid gap-5">
                ...
                <div className="h-20 w-full max-w-xl animate-pulse bg-porcelain" />
                <div className="h-28 w-full max-w-2xl animate-pulse bg-porcelain" />
              </div>
            ) : (
              <>
            ...
            {!isLoading && settings.brandStoryButtonText && settings.brandStoryButtonLink ? (
